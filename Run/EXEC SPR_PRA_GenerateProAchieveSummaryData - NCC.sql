DECLARE @ProviderID INT = 10004762 --Provider Ref of the college
DECLARE @ProviderRef NVARCHAR(50) = 'NCC' --Reference to save into table in case title too long for charts etc.
DECLARE @AcademicYear NVARCHAR(5) = ''

SET @AcademicYear = (SELECT CFG.Value FROM Config CFG WHERE CFG.ConfigID = 'PRA_AcademicYearID')
--SET @AcademicYear = '23/24' --Override
...
DECLARE @OutputTableLocation NVARCHAR(200) = 'KPITracker.dbo.' --Location where the resulting ProAchieve Summary Data table will be created
